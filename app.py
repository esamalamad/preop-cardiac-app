...
with left:
    st.markdown("### 1) Immediate red flags")
st.caption("Choose whichever applies. If none apply, continue to Step 2.")
    ...
    if healthy:
        ...
        if cad:
            ...
            if mace == "<1%":
                ...
                # Only continue down pathway if no final result yet from functional assessment
...
with right:
    ...
    try:
        metric1.metric("RCRI", str(rcri))
        metric2.metric("MACE", mace)
    except Exception:
        metric1.metric("RCRI", "—")
        metric2.metric("MACE", "—")

    try:
        if method == "DASI" and dasi_score is not None:
            metric3.metric("DASI", str(dasi_score))
        else:
            metric3.metric("DASI", "—")
    except Exception:
        metric3.metric("DASI", "—")
